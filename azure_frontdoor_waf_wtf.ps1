...
az group list --query "[].name" -o tsv | ForEach-Object {
  ...
  if ($matches) {
    Write-Host "`n⚠️ VULNERABLE RULES FOUND in policy [$policyName] (RG: $rg)" -ForegroundColor Yellow
    $matches | Format-Table -AutoSize
    $summary += [PSCustomObject]@{ ResourceGroup = $rg; WAFName = $policyName; VulnerableRules = $matches.Count }
  } else {
    $summary += [PSCustomObject]@{ ResourceGroup = $rg; WAFName = $policyName; VulnerableRules = 0 }
  }
}
